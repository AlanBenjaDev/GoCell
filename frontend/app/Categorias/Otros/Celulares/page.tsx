...
export default function CategoriaPage() {
  ...
  useEffect(() => {
    ...
    const fetchProductos = async () => {
      try {
        setLoading(true);
        // Usamos la categoría dinámica
        const res = await fetch(
          `${API_URL}/api/productos/productos/${nombreCategoria}`
        );
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {productos.map((p) => (
            <div key={p.id} className="border rounded-lg p-3 shadow-md">
              <img
                src={p.img_url || "/placeholder.png"}
                alt={p.producto}
                className="w-full h-40 object-cover rounded-md mb-2"
              />
              <h2 className="font-semibold">{p.producto}</h2>
              <p className="text-gray-700">${p.precio.toLocaleString()}</p>
              <button
